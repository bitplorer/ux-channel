...
HEAD: `ux-channel` @ current `main` · `cek-python` **0.2.0**.
...
| 6 | Kernel honesty (cek) | **4** | `[cek]` ≥ 0.2.0 (`cek_host.catalog`). `CekHostCapService` wraps `Host`. Default require (cut #3). `async_verify`. |
