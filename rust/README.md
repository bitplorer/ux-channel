# ux_channel_rs — classic IR 0.1 floor

**Status:** types + wire + CXB + cap verify + classic `Peer` gate + `uxc_check`.
...
| `peer` | Intent → cap **verify** → dispatch → Result (always Result-shaped) | permanent gate |
...
│   ├── nonce.rs         # once/jti store
│   ├── actions.rs       # demo actions (moving)
│   └── bin/
│       ├── uxc_check.rs # conformance runner
│       └── uxc_peer.rs  # HTTP demo peer (verify-only)
└── tests/
    └── integration_peer.rs
```
...
Peer does **not** mint. Mint via Channel / cek-runtime Host (product) or classic `CapService` when that machine is the test subject.
...
| Peer gate | `peer::Peer` (verify-only) | demo `uxc_peer` |
