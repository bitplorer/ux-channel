/**
 * widgets — vendor-island adapters shipped with ux-channel.
 * We wrap another library. Not a design system. Not the protocol.
 *
 * Python: widgets_script_tags() / bridge_script_tags(widgets=True)
 * Load after ux-bridge.js.
 *
 * Keys are npm names: leaflet, codemirror, tom-select, flatpickr,
 * sortablejs, swiper, mermaid, quill, lottie-web.
 *
 * If the page already loaded that library, the adapter uses the global.
 * CDN fetch is only the demo fallback.
 */
(function (global) {
  "use strict";
  if (!global.uxBridge) {
    console.warn("[widgets] uxBridge missing — load ux-bridge.js first");
    return;
  }
  if (global.__UX_WIDGETS_LOADED__) {
    try { console.info("[widgets] re-registering adapters"); } catch (e0) {}
  }
  global.__UX_WIDGETS_LOADED__ = true;
  ...
  function once(key, fn) {
    global.__UX_WIDGET_LOAD__ = global.__UX_WIDGET_LOAD__ || {};
    if (!global.__UX_WIDGET_LOAD__[key]) global.__UX_WIDGET_LOAD__[key] = fn();
    return global.__UX_WIDGET_LOAD__[key];
  }
  ...
  function loadLeaflet() {
    ...
    return once("leaflet", function () {
      ...
      return loadScript("https://unpkg.com/leaflet@1.9.4/dist/leaflet.js").then(function () { return global.L; });
    });
  }
  reg("leaflet", {
    mount: function (el, props) {
      ...
      var handle = {
        ready: loadLeaflet().then(function (L) {
          ...
          L.tileLayer(props.tiles || "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        ...
        _applyMarkers: function (list, fit) {
          ...
          if (fit && markers.length) {
            map.fitBounds(global.L.featureGroup(markers).getBounds().pad(0.2));
          }
        },
        update: function (p) {
          props = p || props;
        ...
        setView: function (center, zoom) { if (map) map.setView(center, zoom != null ? zoom : map.getZoom()); },
        flyTo: function (center, zoom) { if (map) map.flyTo(center, zoom != null ? zoom : map.getZoom()); },
        invalidateSize: function () { if (map) map.invalidateSize(); },
        destroy: function () { if (map) { map.remove(); map = null; } },
      };
      return handle;
    },
  });
  ...
  function loadCM() {
    if (global.CodeMirror) return Promise.resolve(global.CodeMirror);
    return once("codemirror", function () {
      ...
      return loadScript("https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.16/codemirror.min.js").then(function () { return global.CodeMirror; });
    });
  }
  reg("codemirror", {
    mount: function (el, props) {
      ...
      var handle = {
        ...
        update: function (p) { props = p || props; if (cm && p && p.value != null) cm.setValue(p.value); },
        setValue: function (v) { if (cm) cm.setValue(v); },
        getValue: function () { return cm ? cm.getValue() : ta.value; },
        focus: function () { if (cm) cm.focus(); },
        destroy: function () { if (cm) { cm.toTextArea(); cm = null; } el.innerHTML = ""; },
      };
      return handle;
    },
  });
  ...
  function loadTom() {
    if (global.TomSelect) return Promise.resolve(global.TomSelect);
    return once("tom-select", function () {
      ...
      return loadScript("https://cdn.jsdelivr.net/npm/tom-select@2.3.1/dist/js/tom-select.complete.min.js").then(function () { return global.TomSelect; });
    });
  }
  reg("tom-select", {
    mount: function (el, props) {
      ...
      var handle = {
        ...
        update: function (p) {
          ...
          if (p.options) {
            ts.clearOptions();
            p.options.forEach(function (o) { ts.addOption({ value: o.value, text: o.label }); });
        ...
        destroy: function () { if (ts) { ts.destroy(); ts = null; } el.innerHTML = ""; },
      };
      return handle;
    },
  });
  ...
  function loadFp() {
    ...
    return once("flatpickr", function () {
      ...
      return loadScript("https://cdn.jsdelivr.net/npm/flatpickr").then(function () { return global.flatpickr; });
    });
  }
  reg("flatpickr", {
    mount: function (el, props) {
      ...
      var handle = {
        ...
        update: function (p) { props = p || props; if (fp && p && p.value != null) fp.setDate(p.value, true); },
        ...
        destroy: function () { if (fp) { fp.destroy(); fp = null; } el.innerHTML = ""; },
      };
      return handle;
    },
  });
  ...
  function loadSortable() {
    ...
    return once("sortablejs", function () {
      return loadScript("https://cdn.jsdelivr.net/npm/sortablejs@1.15.2/Sortable.min.js").then(function () { return global.Sortable; });
    });
  }
  reg("sortablejs", {
    mount: function (el, props) {
      ...
      function paint(items) {
        ...
        (items || []).forEach(function (it) {
          ...
          li.dataset.id = it.id;
          li.style.cssText = "padding:.75rem 1rem;margin:.35rem 0;border-radius:.75rem;background:rgba(30,41,59,.9);border:1px solid rgba(148,163,184,.2);cursor:grab";
      ...
      var handle = {
        ...
        toArray: function () { return sortable ? sortable.toArray() : []; },
        destroy: function () { if (sortable) { sortable.destroy(); sortable = null; } el.innerHTML = ""; },
      };
      return handle;
    },
  });
  ...
  function loadSwiper() {
    if (global.Swiper) return Promise.resolve(global.Swiper);
    return once("swiper", function () {
      loadCss("https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.css", "ux-swiper-css");
      return loadScript("https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.js").then(function () { return global.Swiper; });
    });
  }
  reg("swiper", {
    mount: function (el, props) {
      ...
      var handle = {
        ready: loadSwiper().then(function (Swiper) {
          sw = new Swiper(el, {
            ...
            navigation: props.navigation === false ? undefined : { nextEl: ".swiper-button-next", prevEl: ".swiper-button-prev" },
    ...
    mount: function (el, props) {
      ...
      var handle = {
        ready: loadMermaid().then(function (mermaid) {
          mermaid.initialize({ startOnLoad: false, theme: props.theme || "dark", securityLevel: props.securityLevel || "strict" });
        ...
        update: function (p) {
          ...
          el.appendChild(pre);
          return loadMermaid().then(function (m) { return m.run({ nodes: [pre] }); });
        },
  ...
  function loadQuill() {
    ...
    return once("quill", function () {
      ...
      return loadScript("https://cdn.jsdelivr.net/npm/quill@2.0.2/dist/quill.js").then(function () { return global.Quill; });
    });
  }
  reg("quill", {
    mount: function (el, props) {
      ...
      var handle = {
        ...
        update: function (p) { props = p || props; if (q && p && p.html != null) q.root.innerHTML = p.html; },
  ...
  reg("lottie-web", {
    mount: function (el, props) {
      ...
      var anim = null;
      function start(lottie) {
        if (anim) { anim.destroy(); anim = null; }
        var opts = { container: el, renderer: props.renderer || "svg", loop: props.loop !== false, autoplay: props.autoplay !== false };
      ...
      var handle = {
        el: el,
        ready: loadLottie().then(start),
        play: function () { if (anim) anim.play(); },
        pause: function () { if (anim) anim.pause(); },
        stop: function () { if (anim) anim.stop(); },
        setSpeed: function (s) { if (anim) anim.setSpeed(s); },
        goToAndPlay: function (frame, isFrame) { if (anim) anim.goToAndPlay(frame, isFrame !== false); },
        update: function (p) {
          props = p || props;
          el.style.background = props.background || "transparent";
          if (global.lottie) return start(global.lottie);
          handle.ready = loadLottie().then(start);
          return handle.ready;
        },
        destroy: function () { if (anim) anim.destroy(); anim = null; el.innerHTML = ""; },
      };
      return handle;
    },
  });
  ...
  try {
    if (global.uxBridge && typeof global.uxBridge.scan === "function") global.uxBridge.scan(document);
  } catch (eScan) {}
